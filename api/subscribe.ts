...
const FROM_EMAIL = process.env.FROM_EMAIL || 'PropiHouse <noreply@propihouse.es>'
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    await resend.emails.send({
      from: FROM_EMAIL,
      to: email,
      subject: 'Te avisaremos en cuanto abramos — PropiHouse',
      html: `
        <div style="font-family:-apple-system,BlinkMacSystemFont,'Inter',sans-serif;background:#000;color:#f5f5f7;padding:48px 32px;max-width:520px;margin:0 auto;border-radius:16px;">
          ...
          <p style="font-size:15px;line-height:1.65;color:rgba(245,245,247,0.6);margin:0 0 32px;">
            Gracias por tu interés en PropiHouse. Cuando abramos, serás de los primeros en saberlo.
          </p>
          ...
          <p style="font-size:12px;color:rgba(245,245,247,0.3);margin:0;">
            PropiHouse · Carrer d'Enric Prat de la Riba, 187 · 08901 L'Hospitalet de Llobregat, Barcelona
          </p>
        </div>
      `,
    })
