...
import { FAQ_FINANCIAR, FAQ_FINANCIAR_CTA } from '../data/faqs'
...
function MortgageCalculator() {
  ...
  /* Builds the jsPDF doc and returns it without saving — used by both
   * the download flow and the "Enviar por email" flow (which encodes
   * the doc as base64 and POSTs it to /api/send-financiar-pdf). */
  const buildPdfDoc = async () => {
    /* Lazy-load jsPDF — keeps the financiar bundle small for users who
     * never click Guardar PDF (it's ~130 KB gzipped on its own). */
  ...
  /* Brand-led WhatsApp share message (not first-person, per Pau's
   * pattern on the Valorador). */
  const handleShareWhatsApp = () => {
    const cuotaStr = noFinancing
      ? 'Sin necesidad de financiación'
      : `Cuota mensual de ${fmtEUR.format(Math.round(monthlyPayment))} a ${plazo} años`
    const text =
      `Simulación de hipoteca en Propi House — ${cuotaStr}. ` +
      `Calcula la tuya: https://www.propihouse.es/financiar`
  ...
  const handleSendEmail = async (e: React.FormEvent) => {
    ...
    try {
      const doc = await buildPdfDoc()
      /* datauristring returns "data:application/pdf;base64,XXXX"; strip the prefix. */
      ...
      const summary = {
        ...
        pctFinanciacion: Number(pctFinanciacion.toFixed(1)),
        debtRatio: Number(debtRatio.toFixed(1)),
        debtLabel,
        noFinancing,
      }
      const resp = await fetch('/api/send-financiar-pdf', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: trimmed, pdfBase64, summary }),
      })
      if (!resp.ok) {
      ...
      console.error('[send-financiar-pdf] error', err)
          ...
          <div className="grid md:grid-cols-2 gap-8 md:gap-6 lg:gap-12 items-start">
            ...
            <div className="relative">
              ...
              <div className="relative bg-white rounded-xl p-7 md:p-6 lg:p-9 shadow-card border border-cream-dark/10">
                ...
                {/* Action row — Guardar PDF + WhatsApp + Enviar por email */}
                <div className="mt-6 pt-6 border-t border-cream-dark/15">
                  <div className="flex flex-wrap gap-2">
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-2 bg-[#25D366]/10 hover:bg-[#25D366] hover:text-white text-[#128C44] text-xs font-bold tracking-wide uppercase px-3.5 py-2.5 rounded-lg transition-colors border border-[#25D366]/30 hover:border-[#25D366] cursor-pointer"
                    ...
                    >
                      ...
                      {emailOpen ? 'Cerrar' : 'Enviar por email'}
                    </button>
                  </div>
                  ...
                  {emailOpen && (
                    <div className="mt-4">
                      {emailStatus === 'sent' ? (
                        <div className="rounded-lg bg-emerald-50 border border-emerald-200 px-4 py-3 text-sm text-emerald-800 leading-relaxed">
                          Te hemos enviado la simulación en PDF. Revisa tu bandeja de entrada.
                        </div>
                      ) : (
                        <form onSubmit={handleSendEmail} className="space-y-2">
                          <label htmlFor="financiar-email" className="text-xs text-text-muted block">
                            Te enviamos el PDF con todos los datos de la simulación.
                          </label>
                          <div className="flex flex-col sm:flex-row gap-2">
                            <input
                              ...
                              className="flex-1 min-w-0 rounded-lg border border-cream-dark/40 bg-warm-white px-4 py-2.5 text-sm text-dark placeholder:text-text-muted/60 focus:outline-none focus:border-blue focus:ring-2 focus:ring-blue/20 transition-colors"
                            />
                            <button
                              type="submit"
                              disabled={emailStatus === 'sending'}
                              className="inline-flex items-center justify-center gap-2 bg-blue hover:bg-blue-dark text-white text-xs font-bold tracking-wide uppercase px-4 py-2.5 rounded-lg transition-colors cursor-pointer disabled:opacity-60 disabled:cursor-wait"
                            >
                              {emailStatus === 'sending' ? 'Enviando…' : 'Enviar PDF'}
                            </button>
                          </div>
...
export default function FinanciarPage() {
  ...
  return (
    <>
      ...
      {/* ═══════════ BLOCK 7b: FAQ ═══════════ */}
      <FAQSection items={FAQ_FINANCIAR} cta={FAQ_FINANCIAR_CTA} />
